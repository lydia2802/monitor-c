...
|---|---|---|
| CLI App | `main.py` | Menu interaktif terminal (pencarian, history, export, dll) |
| REST API + Web Dashboard | `api/server.py`, `run_dashboard.py` | Server Flask: REST API (`/api/v1/...`) + halaman web (`/dashboard`) |
| Analytics Engine | `analytics/dashboard.py` | Statistik real-time, trend, distribusi operator/gender dari SQLite |
| Anomaly Detection | `ml/anomaly_detector.py` | Deteksi pola pencarian anomali |
| User & RBAC | `managers/user_manager.py`, `models/user.py` | Login multi-user dengan role (admin/operator/viewer/auditor) |
...
- **Offline (database lokal)**: set `DATABASE_ENABLED = True`, lalu isi `data/local_database.db` dari sample data:
  ...
  ```bash
  python -c "from utils.database_manager import init_database, import_from_json; init_database(); import_from_json('data/sample_database.json')"
  ```
...
Alternatif: jalankan dari menu CLI (`python main.py` → menu **24. Start API Server**), atau langsung `python -m api.server`.
...
- **Hasil pencarian kosong** → pastikan `config/api_config.py` sudah ada dan `API_ENABLED` atau `DATABASE_ENABLED` bernilai `True`.
